...
def load_wls_params():
    ...
    license_id_raw = os.getenv("LICENSEID")
    ...
    missing = [
        ...
        for name, value in (
            ...
            ("WLSSECRET", secret),
            ("LICENSEID", license_id_raw),
        )
        if not value
    ]
    if missing:
        raise RuntimeError(
            "Missing required Gurobi WLS environment variables: " + ", ".join(missing)
        )

    try:
        license_id = int(license_id_raw)
    except ValueError as e:
        raise RuntimeError("LICENSEID must be an integer.") from e
...
def add_index_selection_constraints(model, x, y, course_indexes):
    for c, groups in course_indexes.items():
        model.addConstr(
            gp.quicksum(y[(c, g)] for g in groups) == x[c], name=f"req_index_{c}"
        )
...
def add_walking_time_constraints(model, y, index_locations, time_slots, locations):
    ...
    for t in time_slots:
        for l in locations:
            occupying_groups = [
                y[(c, g)]
                for (c, g), slots_locs in index_locations.items()
                if (t, l) in slots_locs
            ]
            if occupying_groups:
                model.addConstr(
                    X[t, l] == gp.quicksum(occupying_groups), name=f"link_X_{t}_{l}"
                )
            else:
    ...
    for day_idx in range(6):
        ...
        for i in range(len(day_slots)):
            for j in range(i + 1, len(day_slots)):
                ...
                classes_in_between = gp.quicksum(
                    X[k, l] for k in middle_slots for l in locations
                )
                for l1 in locations:
                    for l2 in locations:
                        if l1 == l2:
...
def optimize_schedule(
    ...
    # --- Build Model ---
    try:
        env = gp.Env(params=load_wls_params())
        model = gp.Model("Course_Optimization", env=env)
    except gp.GurobiError as e:
        raise RuntimeError(
            "Gurobi WLS license initialization failed. Check WLSACCESSID, WLSSECRET, and LICENSEID."
        ) from e
    ...
    X, W = add_walking_time_constraints(
        model, y, index_locations, time_slots_list, locations
    )
